...
const Overview: React.FC<OverviewProps> = ({ instance }) => {
  ...
  const formatarUltimoAcesso = (data?: string) => {
    ...
    if (Number.isNaN(dataConvertida.getTime())) return "Nunca";
  ...
  const formatarTempoJogadoTotal = (segundos?: number) => {
      ...
      <div className="bg-white/5 border border-white/10 rounded-xl p-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p className="text-white font-medium">{formatarUltimoAcesso(instance.last_played)}</p>
          </div>
          <div>
            <label className="text-sm text-white/40">Tempo de jogo total</label>
            <p className="text-white font-medium">
              {formatarTempoJogadoTotal(instance.tempo_total_jogado_segundos)}
            </p>
          </div>
        </div>
      </div>
